...
class DataConsolePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final corrections = subtitles.where((e) => e.isCorrected).length;
            ...
            Padding(
              ...
              child: Row(
                children: [
                  const Text(
                    'Verba 控制台',
                    style: TextStyle(color: VerbaColors.inkWhite, fontSize: 17, fontWeight: FontWeight.w900),
                  ),
                  const Spacer(),
                  GestureDetector(onTap: onCollapse, child: const FloatingIcon(name: 'verba-collapse', size: 28)),
                ],
              ),
            ),
                  ...
                  Row(
                    children: [
                      ...
                      const Expanded(
                        child: Text(
                          'WASAPI 系统音频',
                          style: TextStyle(color: VerbaColors.inkWhite, fontSize: 13, fontWeight: FontWeight.w800),
                        ),
                      ),
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(label, style: const TextStyle(color: VerbaColors.mutedGray, fontSize: 11, fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            Text(value, style: const TextStyle(color: VerbaColors.inkWhite, fontSize: 22, fontWeight: FontWeight.w900)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: danger ? VerbaColors.dangerRed : Colors.white.withValues(alpha: 0.12),
          borderRadius: BorderRadius.circular(18),
        ),
        child: Text(
          label,
          style: const TextStyle(color: VerbaColors.inkWhite, fontSize: 12, fontWeight: FontWeight.w900),
        ),
      ),
    );
  }
